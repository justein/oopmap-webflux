 ...
 */
@Component
public class PrimaryIpService implements IpProxy {

    private static final Logger logger = LoggerFactory.getLogger(PrimaryIpService.class);
    @Value("${ip.service.url}")
    ...
    public Mono<ClientResponse> requestIpInfo(String ip) {
        return WebClient.create(ipServiceUrl).get().uri("/json/{ip}", ip).exchange()
                .doOnError(t -> logger.error("Failed to obtain IP information from primary provider", t));
    }
}
